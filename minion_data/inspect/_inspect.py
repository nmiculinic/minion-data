...
def calc_stats(dp: dataset_pb2.DataPoint) -> pd.DataFrame:
    ...
    items.append(("Signal min", np.min(signal)))
    items.append(("Signal median", np.median(signal)))
    items.append(("Signal max", np.max(signal)))
    items.append(("Signal std", np.std(signal)))
